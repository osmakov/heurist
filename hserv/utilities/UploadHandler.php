...
class UploadHandler
{
    ...
    protected function get_upload_path($file_name = null, $subfolder = null, $version = null) {
    ...
    protected function get_download_url($file_name, $subfolder, $version = null, $direct = false) {
    ...
    protected function is_valid_file_object($file_name, $subfolder) {
    ...
    protected function get_file_object($file_name, $subfolder=null) {
        if ($this->is_valid_file_object($file_name, $subfolder)) {
            ...
            foreach($this->options['image_versions'] as $version => $options) {
                if (!empty($version)) {
                    if (is_file($this->get_upload_path($file_name, $file->subfolder, $version))) {
    ...
    protected function validate($uploaded_file, $file, $error, $index) {
        ...
        if (is_int($this->options['max_number_of_files']) &&
                ...
                !is_file($this->get_upload_path($file->name, $file->subfolder))) {
    ...
    */
    ...
    protected function create_scaled_image($file_name, $subfolder, $version, $options) {
        ...
        return $this->gd_create_scaled_image($file_name, $subfolder, $version, $options);
    }
        ...
        foreach($this->options['image_versions'] as $version => $options) {
            ...
            if ($res===true) {
                if (!empty($version)) {
                    ...
                    if(@$options['scale_to_png']==true && strtolower($type)!='png'){ //ARTEM: scale to png only
                        $new_file_name = str_replace('.'.$type,'.png', $file->name);
                    }else{
                        $new_file_name = $file->name;
                    }
                    $file->{$version.'Name'} = $new_file_name;
                    $file->{$version.'Url'} = $this->get_download_url(
                        $new_file_name,
                        $file->subfolder,
                        $version
    ...
    public function get($print_response = true) {
        ...
        if ($file_name) {
            $response = array(
                $this->get_singular_param_name() => $this->get_file_object($file_name, $subfolder)
            );
        } else {
            $response = array(
